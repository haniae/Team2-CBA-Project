"""Fixed time-period grammar for financial queries with calendar year/quarter defaults."""
...
MONTH_PATTERN = re.compile(r"(?i)\b(Jan|January|Feb|February|Mar|March|Apr|April|May|Jun|June|Jul|July|Aug|August|Sep|September|Oct|October|Nov|November|Dec|December)\s*([12]\d{3})\b")
HALF_YEAR_PATTERN = re.compile(r"(?i)\b(H[12]|[12]H)\s*([12]\d{3})\b")
PERIOD_PATTERN = re.compile(r"(?i)\b(YTD|MTD|QTD)\s*([12]\d{3})\b|\b([12]\d{3})\s*(YTD|MTD|QTD)\b")
ALT_FORMAT_PATTERN = re.compile(r"(?i)\b([12]\d{3})\s*/\s*([12]\d{3})\b|\b([12]\d{3})\s*-\s*(\d{2})\b|\bFY(\d{2})\b|\bCY(\d{2})\b")
...
QUARTER_RANGE_SAME_YEAR_PATTERN = re.compile(r"(?i)\bQ([1-4])\s*[-–—]\s*Q([1-4])\s+([12]\d{3})\b")
...
def parse_periods(text: str, prefer_fiscal: bool = False) -> Dict[str, Any]:
    ...
    CRITICAL FIX: Changed default prefer_fiscal from True to False to default to calendar periods.
    """
    ...
    # Check for current/this/next/previous patterns first
    current_match = CURRENT_PATTERN.search(lower_text)
    if current_match:
        period_type = current_match.group(2).lower()
        is_quarter = period_type.startswith("quarter")
        ...
        warnings.append(f"relative_reference: {current_match.group(1)} {period_type}")
        return {
            "type": "relative",
    ...
    for match in QUARTER_RANGE_PATTERN.finditer(original):
        ...
        if _span_overlaps(span, consumed_spans):
            continue
        q1, q2, year = match.groups()
        year = int(year)
        calendar_override = True  # Quarter ranges default to calendar
        _add_spec(specs, seen_specs, year, year, f"Q{q1}-Q{q2}")
        consumed_spans.append(span)

    # Process quarter cross-year patterns
    for match in QUARTER_CROSS_YEAR_PATTERN.finditer(original):
        span = match.span()
        if _span_overlaps(span, consumed_spans):
            continue
        q1, y1, q2, y2 = match.groups()
        y1, y2 = int(y1), int(y2)
        calendar_override = True  # Quarter cross-year ranges default to calendar
        _add_spec(specs, seen_specs, y1, y2, f"Q{q1}-Q{q2}")
        consumed_spans.append(span)

    # Process quarter patterns with enhanced logic
    for pattern, order in QUARTER_COMBOS:
        for match in pattern.finditer(original):
            ...
            if re.search(r"(?i)\bFY\b", match.group(0)):
                ...
                # CRITICAL FIX: Default quarters to calendar unless explicitly fiscal
                if not fiscal_token_present:
                    calendar_override = True
            _add_spec(specs, seen_specs, year, year, f"Q{quarter_num}")
            consumed_spans.append((start, end))

    # Process range patterns with enhanced logic
    for match in RANGE_PATTERN.finditer(original):
        ...
        if (prefix1 and prefix1.upper() == "FY") or (prefix2 and prefix2.upper() == "FY"):
            ...
            # CRITICAL FIX: Default year ranges to calendar unless explicitly fiscal
            if not fiscal_token_present:
                calendar_override = True
        _add_spec(specs, seen_specs, y1, y2, None)
        consumed_spans.append(start_span)

    # Process fiscal year patterns
    ...
    # Process calendar year patterns
    ...
    # Process month patterns (calendar year)
    for match in MONTH_PATTERN.finditer(original):
        span = match.span()
        if _span_overlaps(span, consumed_spans):
            continue
    ...
    # Process half year patterns (calendar year)
    for match in HALF_YEAR_PATTERN.finditer(original):
        span = match.span()
        if _span_overlaps(span, consumed_spans):
            continue
        year = int(match.group(2))
    ...
    # Process period patterns (calendar year)
    for match in PERIOD_PATTERN.finditer(original):
        span = match.span()
        if _span_overlaps(span, consumed_spans):
            continue
        year_groups = [g for g in match.groups() if g and g.isdigit()]
        if not year_groups:
            continue
        year = int(year_groups[0])
        calendar_override = True
        _add_spec(specs, seen_specs, year, year, None)
        consumed_spans.append(span)

    # Process alternative formats
    for match in ALT_FORMAT_PATTERN.finditer(original):
        span = match.span()
        if _span_overlaps(span, consumed_spans):
            continue
        groups = match.groups()
        if groups[0] and groups[1]:  # 2023/2024 format
            y1, y2 = int(groups[0]), int(groups[1])
            if y2 < y1:
                y1, y2 = y2, y1
            calendar_override = True
            _add_spec(specs, seen_specs, y1, y2, None)
        elif groups[2] and groups[3]:  # 2023-24 format
            year = int(groups[2])
            calendar_override = True
            _add_spec(specs, seen_specs, year, year, None)
        elif groups[4]:  # FY23 format
            year = _convert_two_digit_year(groups[4])
            fiscal_token_present = True
            _add_spec(specs, seen_specs, year, year, None)
        elif groups[5]:  # CY23 format
            year = _convert_two_digit_year(groups[5])
            calendar_override = True
            _add_spec(specs, seen_specs, year, year, None)
        consumed_spans.append(span)

    # Process regular year patterns (CRITICAL FIX: Default to calendar)
    for match in YEAR_PATTERN.finditer(original):
        span = match.span()
        if _span_overlaps(span, consumed_spans):
            continue
        year = int(match.group(1))
        # CRITICAL FIX: Default to calendar year unless explicitly fiscal
        if not fiscal_token_present:
            calendar_override = True
        _add_spec(specs, seen_specs, year, year, None)
        consumed_spans.append(span)
    ...
    # CRITICAL FIX: Default to calendar unless explicitly fiscal
    normalize_to_fiscal = prefer_fiscal
    if calendar_override:
        ...
        # CRITICAL FIX: Default to calendar when no explicit indicator
        normalize_to_fiscal = False
    ...
    # CRITICAL FIX: Default to calendar granularity
    if calendar_override:
    ...
    quarter_range_spec = next((spec for spec in specs_sorted if spec.get("quarter") and "-" in str(spec.get("quarter"))), None)

    if quarter_range_spec is not None:
        period_type = "range"
        # For quarter ranges, create proper range items
        if quarter_range_spec["start"] != quarter_range_spec["end"]:
            items = [
                _spec_to_item({"start": quarter_range_spec["start"], "end": quarter_range_spec["start"], "quarter": quarter_range_spec.get("quarter")}),
                _spec_to_item({"start": quarter_range_spec["end"], "end": quarter_range_spec["end"], "quarter": quarter_range_spec.get("quarter")}),
            ]
        else:
            # Same year quarter range
            items = [
                _spec_to_item({"start": quarter_range_spec["start"], "end": quarter_range_spec["start"], "quarter": quarter_range_spec.get("quarter")}),
                _spec_to_item({"start": quarter_range_spec["end"], "end": quarter_range_spec["end"], "quarter": quarter_range_spec.get("quarter")}),
            ]
    elif range_spec is None and len(specs_sorted) == 1:
        period_type = "single"
