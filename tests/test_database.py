...
def test_default_deliverables_seeded(temp_db: Path) -> None:
    deliverables = database.list_deliverables(temp_db)
    assert len(deliverables) == len(database.DEFAULT_DELIVERABLES)
    titles = [item.title for item in deliverables]
    assert "Select Financial Datasets (SEC EDGAR, Yahoo Finance, Kaggle, etc.)" in titles
...
def test_update_deliverable_status(temp_db: Path, position: int, status: str) -> None:
    updated = database.update_deliverable_status(temp_db, position, status)
    assert updated.status == status
    assert updated.position == position
    reloaded = {
        item.position: item.status for item in database.list_deliverables(temp_db)
    }
    assert reloaded[position] == status
