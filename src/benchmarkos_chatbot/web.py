"""FastAPI service exposing the BenchmarkOS chatbot, analytics, and web UI."""
...
app = FastAPI(title="BenchmarkOS Analyst Copilot", version="1.0.0")
BASE_DIR = Path(__file__).resolve().parents[2]
FRONTEND_DIR = (BASE_DIR / "webui").resolve()
...
def metrics(
    ...
    start_year: Optional[int] = Query(
        None, description="Optional inclusive start of fiscal year range"
    ),
    end_year: Optional[int] = Query(
        None, description="Optional inclusive end of fiscal year range"
    ),
    ...
    for ticker in ticker_list:
        ...
        period_label = _summarise_period(engine._period_span(rec.period) for rec in chosen.values())
        responses.append(
            MetricsResponse(
                ticker=ticker,
...
def facts(
    ...
    metric: Optional[str] = Query(None, description="Optional metric name to filter"),
) -> FactsResponse:
    engine = get_engine()
