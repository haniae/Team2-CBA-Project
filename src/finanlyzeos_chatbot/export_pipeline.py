...
def _safe_slug(value: str, fallback: str = "finanlyzeos") -> str:
...
def _build_pdf(payload: Dict[str, Any]) -> bytes:
    ...
    # Extract metadata
    company = meta.get("company") or meta.get("ticker") or "FinanlyzeOS Dashboard"
...
def generate_dashboard_export(engine: "AnalyticsEngine", ticker: str, fmt: str) -> ExportResult:
    ...
    if format_normalized in {"pdf"}:
        ...
        filename = f"FinanlyzeOS_{ticker_label}_{today}.pdf"
    ...
    if format_normalized in {"ppt", "pptx"}:
        ...
        filename = f"FinanlyzeOS_{ticker_label}_{today}.pptx"
        return ExportResult(
            content=content,
    ...
    if format_normalized in {"xlsx", "excel"}:
        ...
        filename = f"FinanlyzeOS_{ticker_label}_{today}.xlsx"
        return ExportResult(
            content=content,
...
def generate_multi_ticker_export(
    ...
    zip_buffer.seek(0)
    zip_filename = f"FinanlyzeOS_MultiCompany_{len(tickers)}companies_{today}.zip"
...
def _build_comparative_summary_pdf(engine: AnalyticsEngine, tickers: List[str]) -> bytes:  # type: ignore
    ...
    pdf.set_font("Helvetica", "B", 10)
    pdf.set_text_color(*COLOR_PRIMARY)
    pdf.cell(0, 6, "Generated by FinanlyzeOS", align="C", ln=1)
